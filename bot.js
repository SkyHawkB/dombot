                ...
                .asCallback(function(err,row) {
                    if(err) drawCallback(err,kingdom,msgCallback)
                        ...
                        if(row==undefined) {
                            ...
                            var shelters=gameInfo.gameParameters.setupInstructions.usesShelters;
...
function drawKingdom(err, kingdom, msgCallback) {
    if(!err) {
        ...
        var cardSupply=fullInfo.filter(function(x){if(x.type != 'Event' && x.type != 'Landmark' && x.type != 'Project' && x.type != 'Shelter') return kingdom.includes(x.nicename)});
        var csoSupply=fullInfo.filter(function(x){if(x.type == 'Event' || x.type == 'Landmark' || x.type == 'Project' || x.type=='Shelter') return kingdom.includes(x.nicename)});
        ...
        // Handle Knights/Castles sorting here? Or add sort to JSON? Probably just handle it here.
        const costSort = function(a,b) {
            if(a.cost.coins<0) a.cost.coins++;
            if(b.cost.coins<0) b.cost.coins++;
            if(a.cost.coins==b.cost.coins)
                if(a.cost.potion==b.cost.potion)
                    if(a.cost.debt==b.cost.debt) {
                        var sortA=a.nicename;
                        var sortB=b.nicename;
                        if(a.nicename=='shelters')
                            sortA=' shelters';
                        if(b.nicename=='shelters')
                            sortB=' shelters';
                        if(a.nicename.startsWith('sir-')||a.nicename.startsWith('dame-'))
                            sortA='knight';
                        if(b.nicename.startsWith('sir-')||b.nicename.startsWith('dame-'))
                            sortB='knight';
                        if(a.nicename=='humble-castle')
                            sortA='castle';
                        if(b.nicename=='humble-castle')
                            sortB='castle';
            ...
            knex('users')
                ...
                .then(function(user1Row) {
                    if(!user1Row) {
                        ...
                        if(users[0]=='Lord Rattington') {
                            logger.info('Player 1 is Lord Rattington');
                            players.push({id:1073741824,name:'Lord Rattington',wins:0});
                        } else  {
                            logger.info('Player 1 id: '+user1Row.id);
                        ...
                        return knex('users').where('name',users[1]).first('id','name').then(function(user2Row) {
                            if(!user2Row) {
                                ...
                                if(users[1]=='Lord Rattington') {
                                    logger.info('Player 2 is Lord Rattington');
                                    players.push({id:1073741824,name:'Lord Rattington',wins:0});
                                } else  {
                                    logger.info('Player 2 id: '+user2Row.id);
                                ...
                                return knex('gameresults').whereNotNull('gameresult').orderBy('gameid','desc').first('gameid').then(function(gameIdRow) {
                                    if(!gameIdRow) {
                                        ...
                                        return knex('gameresults').whereNotNull('gameresult').andWhere('gameid','>',Math.max(gameIdRow.gameid-10000,maxGameId-1000))
                                    .select('gameid','pregame','gameresult').then(function(rows) {
                                        ...
                                        if(games.length>0) {
                                            ...
                                            message.author.send('No recent games found between those two players.');
                                        }
    ...
    if(msg.startsWith(prefix+'text')) {
        if(nicify(msg.replace(prefix+'text',''))=='help') {
            ...
            if(!cardText.length>0) {
                message.author.send('No text found for that card');
            } else {
                var color='14342874';
                switch(info.type) {
                    case 'Project':
                ...
                if(info) {
                    if(cardText.indexOf("---")>0) {
                        ...
                        var embed = new Discord.RichEmbed()
                            ...
                            .setDescription(aboveLine)
                            .addField("__                          __",belowLine)
                            //.setThumbnail("./images/cards/"+cardname+".jpg");
                            .setThumbnail("attachment://image.png");
                        //        message.channel.send({embed});
                        message.channel.send({embed, files:[{attachment:'./images/cards/'+cardname+'.jpg',name:'image.png'}]});
                        //     message.channel.send({embed:{   
                        //       color: color,
                        //       title: info.name,
        ...
        if(nicify(msg.replace(prefix+'leader',''))=='help') {
            ...
            if(!leaderCache.has(sortBy)) {
                ...
                knex.from('ratinghistory')
                    ...
                    .select('users.name', 'ratinghistory.*').limit(30000).orderByRaw(orderByClause).then(function(rows) {
                        if(rows.length>0) {
                            // Look for ids 4,245,2249 in leaderboard; only cache if all are present
                            // If these ids are missing, it's likely that the leaderboard is being calculated and currently incomplete
                            leaderCache.set(sortBy,rows);
                            var j=0;
                            var period=rows[0].period;
